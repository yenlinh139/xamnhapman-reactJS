            ...
            <div className="stats-grid">
              {dashboard.weather_summary.rainfall && (
                <>
                  <div className="stat-item">
                    ...
                    <span className="stat-value">{dashboard.weather_summary.rainfall.total || 0}mm</span>
                  </div>
                  <div className="stat-item">
                    <span className="stat-label">Mưa TB/ngày</span>
                    <span className="stat-value">{dashboard.weather_summary.rainfall.daily_average || 0}mm</span>
                  </div>
                  <div className="stat-item">
              ...
              {dashboard.weather_summary.temperature && (
                <div className="stat-item">
                  ...
                  <span className="stat-value">{dashboard.weather_summary.temperature.average || 0}°C</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="alerts-card dashboard-card">
          ...
          <div className="alert-summary">
            {dashboard.alerts && dashboard.alerts.count > 0 ? (
              <span className="has-alerts">⚠️ {dashboard.alerts.description || `${dashboard.alerts.count} cảnh báo`}</span>
            ) : (
          ...
          <div className="coverage-card dashboard-card">
            ...
            <div className="coverage-info">
              ...
              {dashboard.data_coverage.weather?.last_update && (
                <p>Cập nhật: {new Date(dashboard.data_coverage.weather.last_update).toLocaleDateString('vi-VN')}</p>
              )}
            </div>
          </div>
        )}
            ...
            <div key={station.code} className="station-card">
              ...
              <div className="station-stats">
                ...
                <div className="stat">
                  ...
                  <span className="value">{station.rainyDays} ngày ({station.percentage.toFixed(1)}%)</span>
                </div>
              </div>
  ...
  return (
    <div className={`hydro-summary ${className}`}>
      <div className="summary-header">
        ...
        {startDate && endDate && (
          <p>Từ {new Date(startDate).toLocaleDateString('vi-VN')} đến {new Date(endDate).toLocaleDateString('vi-VN')}</p>
        )}
      </div>
        ...
        {data.summary && (
          <div className="summary-card">
            ...
            <div className="summary-stats">
              <p>Tổng số trạm: <strong>{data.summary.total_stations || 0}</strong></p>
              <p>Bản ghi khí tượng: <strong>{data.summary.total_weather_records || 0}</strong></p>
              <p>Bản ghi thủy văn: <strong>{data.summary.total_hydro_records || 0}</strong></p>
            </div>
          </div>
        )}
          ...
          <div className="summary-card">
            ...
            <div className="summary-stats">
              {data.weather.rainfall && (
                <>
                  <p>Mưa TB toàn vùng: <strong>{data.weather.rainfall.average_total || 0}mm</strong></p>
                  <p>Mưa lớn nhất: <strong>{data.weather.rainfall.maximum_total || 0}mm</strong></p>
                </>
              )}
              {data.weather.temperature && (
                <>
                  <p>Nhiệt độ TB: <strong>{data.weather.temperature.average || 0}°C</strong></p>
                  <p>Nhiệt độ cao nhất: <strong>{data.weather.temperature.maximum || 0}°C</strong></p>
                </>
              )}
            </div>
          </div>
          ...
          <div className="summary-card">
            ...
            <div className="summary-stats">
              {data.hydrology.water_level_nb && (
                <>
                  <p>Mực nước TB (Nhà Bè): <strong>{data.hydrology.water_level_nb.average || 0}cm</strong></p>
                  <p>Mực nước cao nhất (Nhà Bè): <strong>{data.hydrology.water_level_nb.maximum || 0}cm</strong></p>
                </>
              )}
              {data.hydrology.water_level_pa && (
                <>
                  <p>Mực nước TB (Phú An): <strong>{data.hydrology.water_level_pa.average || 0}cm</strong></p>
                  <p>Mực nước cao nhất (Phú An): <strong>{data.hydrology.water_level_pa.maximum || 0}cm</strong></p>
                </>
              )}
            </div>
          </div>
