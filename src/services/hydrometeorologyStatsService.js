  ...
  const fetchRainfallStats = useCallback(async () => {
    ...
    try {
      if (DISABLE_HYDRO_STATS_API) {
        ...
        const mockData = [
          { station_name: 'Trạm Nhà Bè', station_code: 'NB001', total_rainfall: 156.7, avg_rainfall: 5.2, rainy_days: 19, rainy_days_percentage: 63.3 },
          { station_name: 'Trạm Phú An', station_code: 'PA001', total_rainfall: 142.3, avg_rainfall: 4.7, rainy_days: 17, rainy_days_percentage: 56.7 },
          { station_name: 'Trạm Thủ Đức', station_code: 'TD001', total_rainfall: 134.8, avg_rainfall: 4.5, rainy_days: 16, rainy_days_percentage: 53.3 }
        ];
        setData(mockData);
        return;
      }
  ...
  const fetchWaterLevelStats = useCallback(async () => {
    ...
    try {
      if (DISABLE_HYDRO_STATS_API) {
        ...
        const mockData = [
          { station_name: 'Trạm Nhà Bè', station_code: 'NB001', avg_water_level: 125.3, max_water_level: 145.7, min_water_level: 98.2 },
          { station_name: 'Trạm Phú An', station_code: 'PA001', avg_water_level: 98.7, max_water_level: 112.4, min_water_level: 76.8 },
          { station_name: 'Trạm Cần Giờ', station_code: 'CG001', avg_water_level: 87.5, max_water_level: 95.3, min_water_level: 72.1 }
        ];
        setData(mockData);
        return;
      }
      ...
      const response = await axiosInstance.get(`/hydrometeorology-stats/water-level-by-station?${params.toString()}`);
      setData(response.data.data || response.data);
    } catch (err) {
    ...
    try {
      if (DISABLE_HYDRO_STATS_API) {
        ...
        const mockDashboard = {
          ...
          recent_data: [
            { station: 'Nhà Bè', rainfall: 12.5, water_level: 125.3, timestamp: '2022-09-30T15:00:00Z' },
            { station: 'Phú An', rainfall: 8.2, water_level: 98.7, timestamp: '2022-09-30T15:00:00Z' },
            { station: 'Thủ Đức', rainfall: 15.1, water_level: 87.5, timestamp: '2022-09-30T15:00:00Z' }
          ]
        };
...
export const useWeatherAlerts = (alertType = 'all', days = 7) => {
  ...
  const fetchAlerts = useCallback(async () => {
    ...
    try {
      ...
      const startDate = days <= 7 ? '2022-09-24' : 
                       days <= 30 ? '2022-09-01' : '2022-08-01';
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      if (DISABLE_HYDRO_STATS_API) {
        ...
        const mockData = [
          { period: '2022-09', rainfall: 245.6, avg_temperature: 28.5, avg_humidity: 82.3, total_stations: 24 },
          { period: '2022-08', rainfall: 198.2, avg_temperature: 29.1, avg_humidity: 79.8, total_stations: 24 },
          { period: '2022-07', rainfall: 156.4, avg_temperature: 29.8, avg_humidity: 77.2, total_stations: 23 }
        ];
        setData(mockData);
        return;
      }
  ...
  const fetchStations = useCallback(async () => {
    ...
    try {
      if (DISABLE_HYDRO_STATS_API) {
        ...
        const mockStations = [
          { id: 'NB001', name: 'Trạm Nhà Bè', code: 'NB001', latitude: 10.6967, longitude: 106.7589, type: 'hydromet', status: 'active' },
          { id: 'PA001', name: 'Trạm Phú An', code: 'PA001', latitude: 10.8142, longitude: 106.6438, type: 'hydromet', status: 'active' },
          { id: 'TD001', name: 'Trạm Thủ Đức', code: 'TD001', latitude: 10.8508, longitude: 106.7717, type: 'hydromet', status: 'active' },
          { id: 'CG001', name: 'Trạm Cần Giờ', code: 'CG001', latitude: 10.4103, longitude: 106.9533, type: 'hydromet', status: 'active' }
        ];
